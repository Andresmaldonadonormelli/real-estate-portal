...
export default function Dashboard() {
  ...
  const actionItems=useMemo(()=>{
    const items:{id:string;kind:'rent'|'document';title:string;detail:string;propertyId?:string;days?:number}[]=[];
  ...
  },[documents,pendingRents,properties,monthLabel]);
  ...
  return <div className="dashboard-page" style={{padding:24,maxWidth:1200,margin:'0 auto'}}>
    ...
    {loading?<PageSkeleton variant="dashboard"/>:<>
      ...
      {actionItems.length>0&&<section className="action-center card"><div className="section-heading-row"><div><div className="eyebrow">NEEDS YOU</div><h2>Action Center</h2></div>{actionItems.length>3&&<span className="muted-small">{actionItems.length} open</span>}</div><div className="action-list">{actionItems.slice(0,3).map(item=><button key={item.id} className="action-row" onClick={()=>{if(item.kind==='rent'&&item.propertyId){setReviewPropertyId(item.propertyId);setTestPreview(false);}else router.push('/ledger');}}><span className={`action-dot ${item.days!==undefined&&item.days<=7?'urgent':''}`}/><span><strong>{item.title}</strong><small>{item.detail}</small></span><span className="action-arrow">→</span></button>)}</div>{actionItems.length>3&&<button className="text-action" onClick={()=>router.push('/actions')}>View all actions →</button>}</section>}
      <section className="cashflow-card card"><div className="section-heading-row cashflow-head"><div><h2>Cash Flow</h2><p>Trailing 12 months · posted ledger activity</p></div><div className="cashflow-controls"><select aria-label="Cash flow property" value={cashPropertyId} onChange={e=>setCashPropertyId(e.target.value)}><option value="">All properties</option>{properties.map(p=><option key={p.id} value={p.id}>{p.address}</option>)}</select><div className="segmented"><button className={cashMode==='monthly'?'active':''} onClick={()=>setCashMode('monthly')}>Monthly</button><button className={cashMode==='cumulative'?'active':''} onClick={()=>setCashMode('cumulative')}>Cumulative</button></div></div></div><CashFlowChart rows={cashFlow}/></section>
      ...
      <div className="recent-activity-heading"><h2 style={{fontSize:19,fontWeight:600}}>Recent Activity</h2></div><div className="card recent-activity-card"><div className="recent-activity-list">{transactions.filter(t=>(t.status||'posted')==='posted').slice(0,6).map(tx=>{const property=properties.find(p=>p.id===tx.property_id);const unit=tx.unit_id?unitMap[tx.unit_id]:undefined;return <button type="button" className="recent-activity-row" key={tx.id} onClick={()=>router.push('/ledger')} aria-label={`Open ${tx.description} in ledger`}>{imageUrls[tx.property_id]?<img src={imageUrls[tx.property_id]} alt="" className="recent-activity-thumb"/>:<div className="recent-activity-thumb recent-activity-thumb-fallback">⌂</div>}<div className="recent-activity-copy"><strong>{property?.address||'Portfolio activity'}</strong><span>{tx.description}{unit?.unit_number?` · Unit ${unit.unit_number}`:''} · {new Date(`${tx.transaction_date}T12:00:00`).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span></div><strong className={tx.type==='income'?'amount-positive':tx.type==='expense'?'amount-negative':''}>{tx.type==='expense'?'-':''}{formatCurrency(Math.abs(tx.amount))}</strong></button>})}</div><Link href="/ledger" className="pill-link primary-action recent-ledger-button">Open ledger</Link></div>
    </>}
