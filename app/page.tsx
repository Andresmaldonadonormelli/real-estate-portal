...
export default function Dashboard() {
  ...
  const actionItems=useMemo(()=>{
    ...
    const posted=transactions.filter(tx=>(tx.status||'posted')==='posted');const newImports=posted.filter(tx=>tx.is_new_import);const needsCategory=posted.filter(tx=>(tx as Transaction & {needs_review?:boolean}).needs_review||tx.category==='Needs Review');
