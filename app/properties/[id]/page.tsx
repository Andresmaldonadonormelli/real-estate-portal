...
function Overview({property,units,transactions,documents,expectedRent,metrics,onNavigate}:{property:Property;units:Unit[];transactions:Tx[];documents:PropertyDocument[];expectedRent:number;metrics:ReturnType<typeof calculateMetrics>;onNavigate:(tab:Tab)=>void;onPropertyUpdated:(patch:Record<string,unknown>)=>void}){
  const [period,setPeriod]=useState<HistoryPeriod>('1Y'); const [rentReviewOpen,setRentReviewOpen]=useState(false); const [confirmingRent,setConfirmingRent]=useState(false);
  ...
  const pendingRentRows=transactions.filter(tx=>tx.status==='pending'&&tx.category==='Rent'); const pendingRent=pendingRentRows.length; async function confirmPropertyRents(){setConfirmingRent(true);for(const tx of pendingRentRows)await supabase.from('transactions').update({status:'posted'}).eq('id',tx.id);location.reload()}
  ...
  const pulseSignals=fullyVacant?[
    {key:'vacancy',label:'Vacant for',value:`${vacancyDays||0} days`,detail:'',tone:'negative',priority:1,action:()=>onNavigate('units')},
    {key:'holding',label:'Holding costs',value:`${formatKpiCurrency(holdingCosts)} this month`,detail:'',tone:'negative',priority:2,action:()=>location.href=`/ledger?property=${property.id}`},
    {key:'expenses',label:'Expenses',value:expenseStatus,detail:'',tone:expenseTone,priority:3,action:()=>location.href=`/ledger?property=${property.id}`},
  ...
  return <div className="property-overview-pulse"><div className="property-overview-layout">
    ...
    <aside className="property-pulse-rail"><div className="property-pulse-head"><h2>Property Pulse</h2><span>Updated {pulseUpdatedAt}</span></div><div className="property-pulse-summary"><strong>{pulseHeadline}</strong><span>{pulseExplanation}</span></div><div className="property-pulse-rent-progress" aria-label={`${rentProgress}% of expected rent confirmed; ${expectedByToday}% expected by today`}><i><b style={{width:`${rentProgress}%`}}/></i><span><b>{formatKpiCurrency(collectedRent)} confirmed</b><small>{formatKpiCurrency(expectedRent)} expected</small></span>{pendingRent>0&&<button type="button" className="property-pulse-confirm-rent" onClick={()=>setRentReviewOpen(true)}>Confirm rent</button>}</div><div className="property-pulse-list">{pulseSignals.map(signal=><button type="button" onClick={signal.action} key={signal.key} className={`property-pulse-row is-${signal.tone}`}><span>{signal.label}</span><strong>{signal.value}</strong></button>)}</div></aside>
  </div>{rentReviewOpen&&<div className="property-rent-dialog-backdrop" role="presentation" onMouseDown={()=>setRentReviewOpen(false)}><div className="property-rent-dialog" role="dialog" aria-modal="true" aria-labelledby="property-rent-dialog-title" onMouseDown={event=>event.stopPropagation()}><div><h2 id="property-rent-dialog-title">Confirm rent</h2><button type="button" onClick={()=>setRentReviewOpen(false)} aria-label="Close">×</button></div><p>Confirm {pendingRent} pending rent {pendingRent===1?'payment':'payments'} for this property.</p><div className="property-rent-dialog-actions"><button type="button" className="secondary-pill" onClick={()=>setRentReviewOpen(false)}>Cancel</button><button type="button" className="primary-action" disabled={confirmingRent} onClick={()=>void confirmPropertyRents()}>{confirmingRent?'Confirming…':'Confirm received'}</button></div></div></div>}</div>;
}
