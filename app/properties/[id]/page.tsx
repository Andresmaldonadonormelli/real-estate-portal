...
function MortgageOverview({property,onUpdated}:{property:Property;onUpdated:(patch:Record<string,unknown>)=>void}){
  ...
  async function save(){setSaving(true);setError('');const patch={monthly_mortgage_payment:payment||0,mortgage_balance:Number(form.balance||0)||0,mortgage_due_day:Math.min(28,Math.max(1,Number(form.dueDay||1))),mortgage_recurring_enabled:form.recurring,mortgage_principal_amount:Number(form.principal||0)||null,mortgage_interest_amount:Number(form.interest||0)||null,mortgage_escrow_amount:Number(form.escrow||0)||null};const r=await supabase.from('properties').update(patch).eq('id',property.id);setSaving(false);if(r.error){setError(r.error.message);return;}onUpdated(patch);setEditing(false);}
  return <section className="card property-panel mortgage-overview-panel">
    ...
    {!editing?<div className="mortgage-overview-summary"><div><span>Monthly payment</span><strong>{payment?formatCurrency(payment):'Not set'}</strong></div><div><span>Current balance</span><strong>{Number(form.balance||0)>0?formatKpiCurrency(Number(form.balance)):'Not set'}</strong></div><div><span>Schedule</span><strong>{form.recurring?`Monthly · day ${form.dueDay}`:'Paused'}</strong></div><div><span>Payment split</span><strong>{complete?'Complete':'Needs setup'}</strong><small>{complete?`${formatCurrency(Number(form.principal||0))} principal · ${formatCurrency(Number(form.interest||0))} interest · ${formatCurrency(Number(form.escrow||0))} escrow`:'Add principal, interest and escrow so recurring payments post already reviewed.'}</small></div></div>:
