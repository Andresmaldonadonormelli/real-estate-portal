...
function Overview({property,units,transactions,documents,expectedRent,metrics,onNavigate}:{property:Property;units:Unit[];transactions:Tx[];documents:PropertyDocument[];expectedRent:number;metrics:ReturnType<typeof calculateMetrics>;onNavigate:(tab:Tab)=>void;onPropertyUpdated:(patch:Record<string,unknown>)=>void}){
  ...
  const priorCashFlow=history.slice(0,-1).reduce((sum,row)=>sum+row.cashFlow,0)/Math.max(1,history.length-1);
  const currentCashFlow=current?.cashFlow||0;
  const cashPace=priorCashFlow?Math.round((currentCashFlow/priorCashFlow-1)*100):0;
  ...
  const pulseHeadline=rentOutstanding>0?'Rent needs confirmation':leaseDays!=null&&leaseDays<=90?'Lease ending soon':currentRepairs>recentAverage&&recentAverage>0?'Expenses need attention':'Fully occupied';
  const pulseExplanation=rentOutstanding>0?`${formatCurrency(expectedRent)} expected, but ${collectedRent?`${formatCurrency(collectedRent)} is`:'none'} confirmed.`:leaseDays!=null&&leaseDays<=90?`${nextLease?.unit.unit_number||'A unit'} has ${leaseDays} days remaining.`:currentRepairs>recentAverage&&recentAverage>0?`Repairs are ${Math.round((currentRepairs/recentAverage-1)*100)}% above the recent average.`:`All ${units.length} units are occupied.`;
  ...
  return <div className="property-overview-pulse"><div className="property-overview-layout">
    ...
    <aside className="property-pulse-rail"><div className="property-pulse-head"><h2>Property Pulse</h2><span>Updated today</span></div><div className="property-pulse-primary"><strong>{pulseHeadline}</strong><span>{pulseExplanation}</span><i><b style={{width:`${rentProgress}%`}}/></i></div><div className="property-pulse-list"><div><span>Rent confirmation</span><strong className={rentOutstanding>0?'amount-negative':'amount-positive'}>{formatCurrency(collectedRent)} of {formatCurrency(expectedRent)}</strong></div><div><span>Occupancy</span><strong className={occupied===units.length&&units.length?'amount-positive':occupied<units.length?'amount-negative':''}>{occupied} of {units.length}</strong><small>{leaseDays!=null&&leaseDays<=90?`Lease risk in ${leaseDays} days`:'No upcoming lease risk'}</small></div><div><span>Cash-flow pace</span><strong className={cashPace<0?'amount-negative':'amount-positive'}>{priorCashFlow?`${Math.abs(cashPace)}% ${cashPace<0?'below':'above'} typical`:'Building history'}</strong></div><div><span>Expenses</span><strong className={currentRepairs>recentAverage&&recentAverage>0?'amount-negative':'amount-positive'}>{currentRepairs>recentAverage&&recentAverage>0?`${Math.round((currentRepairs/recentAverage-1)*100)}% above average`:'Within recent range'}</strong></div></div></aside>
  </div></div>;
}
