...
function Overview({property,units,transactions,documents,expectedRent,metrics,onNavigate}:{property:Property;units:Unit[];transactions:Tx[];documents:PropertyDocument[];expectedRent:number;metrics:ReturnType<typeof calculateMetrics>;onNavigate:(tab:Tab)=>void;onPropertyUpdated:(patch:Record<string,unknown>)=>void}){
  ...
  const priorRows=history.slice(0,-1), priorCashFlow=priorRows.reduce((sum,row)=>sum+row.cashFlow,0)/Math.max(1,priorRows.length);
  ...
  const pulseSignals=[
    {key:'rent',label:'Rent',value:`${formatCurrency(collectedRent)} / ${formatCurrency(expectedRent)}`,detail:rentLate?`${formatCurrency(rentOutstanding)} remains after the due date.`:rentProgress===100?'All expected rent is confirmed.':isPastRentDue?'Rent confirmation is still in progress.':'Rent is not due yet.',tone:rentLate?'negative':rentProgress===100?'positive':'neutral',priority:rentLate?1:rentProgress===100?4:2},
    {key:'occupancy',label:'Occupancy',value:`${occupied}/${units.length} occupied`,detail:leaseDays!=null&&leaseDays<=90?`Lease ending in ${leaseDays} days.`:'No leases ending within 90 days.',tone:occupied<units.length?'negative':leaseDays!=null&&leaseDays<=90?'warning':'neutral',priority:occupied<units.length?1:leaseDays!=null&&leaseDays<=90?2:5},
    {key:'cash',label:'Cash flow',value:hasCashHistory?`${cashDelta<0?'−':''}${formatCurrency(Math.abs(cashDelta))} ${cashDelta<0?'vs typical':'above typical'}`:'Not enough history',detail:hasCashHistory?`${formatCurrency(currentCashFlow)} this month.`:'Not enough history for comparison.',tone:cashBelowTypical?'negative':hasCashHistory&&cashDelta>baselineThreshold?'positive':'neutral',priority:cashBelowTypical?1:hasCashHistory&&cashDelta>baselineThreshold?4:3},
    ...(expenseUnusual?[{key:'expenses',label:'Expenses',value:`${formatCurrency(currentRepairs-recentAverage)} above average`,detail:'Repairs are meaningfully above the recent monthly average.',tone:'warning',priority:1}]:[]),
  ...
  return <div className="property-overview-pulse"><div className="property-overview-layout">
    <main className="property-overview-main">
      <section className="property-overview-chart-open">
        ...
        <div className={`property-chart-periods ${periodCashFlow<0?'is-negative':'is-positive'}`} aria-label="Chart period">{(['3M','6M','9M','1Y'] as HistoryPeriod[]).map(value=><button key={value} className={period===value?'active':''} onClick={()=>setPeriod(value)}>{value}</button>)}</div>
      </section>
    <ActionCenter items={actionItems} title="Property Action Center" showEyebrow={false} hideWhenEmpty/>
    ...
    <aside className="property-pulse-rail"><div className="property-pulse-head"><h2>Property Pulse</h2><span>Updated {pulseUpdatedAt}</span></div><div className="property-pulse-summary"><strong>{pulseHeadline}</strong><span>{pulseExplanation}</span></div><div className="property-pulse-rent-progress" aria-label={`${rentProgress}% of expected rent confirmed`}><i><b style={{width:`${rentProgress}%`}}/><em style={{left:`${expectedByToday}%`}}/></i><span><b>{formatCurrency(collectedRent)} confirmed</b><small>{formatCurrency(expectedRent)} expected</small></span></div><div className="property-pulse-list">{pulseSignals.map(signal=><div key={signal.key} className={`property-pulse-row is-${signal.tone}`}><span>{signal.label}</span><strong>{signal.value}</strong><small>{signal.detail}</small></div>)}</div></aside>
  </div></div>;
}
