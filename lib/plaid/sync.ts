...
async function materialize(db:SupabaseClient,item:PlaidItem,row:any,account:LinkedAccount){
  ...
  const type=primary.startsWith('TRANSFER_')?'transfer':row.amount>0?'expense':'income';
  const amount=type==='expense'?-Math.abs(row.amount):type==='income'?Math.abs(row.amount):Number(row.amount);
  ...
  const payload={user_id:item.user_id,property_id:account.property_id,unit_id:null,transaction_date:row.date,type,category:'Needs Review',description:row.name||'Bank transaction',payee_source:row.merchant_name||row.name||null,amount,notes:'Imported from connected bank account',source:'plaid',source_institution:item.institution_name,source_account_mask:account.mask,source_connection_status:'connected',import_key:`plaid:${row.transaction_id}`,status,confirmed_at:status==='posted'?new Date().toISOString():null,needs_review:true};
  ...
  if(existing){const update=existing.needs_review?payload:{transaction_date:row.date,amount,status,confirmed_at:status==='posted'?new Date().toISOString():null,source_connection_status:'connected'};ledger=(await db.from('transactions').update(update).eq('id',existing.id).select('id').single()).data;}
