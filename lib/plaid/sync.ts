...
async function materialize(db:SupabaseClient,item:PlaidItem,row:any,account:LinkedAccount){
  ...
  if(existing){const {is_new_import:ignoredNew,import_acknowledged_at:ignoredAt,...reviewUpdate}=payload;const update=existing.needs_review?reviewUpdate:{transaction_date:row.date,amount,status,confirmed_at:status==='posted'?new Date().toISOString():null,source_connection_status:'connected'};ledger=(await db.from('transactions').update(update).eq('id',existing.id).select('id').single()).data;}
