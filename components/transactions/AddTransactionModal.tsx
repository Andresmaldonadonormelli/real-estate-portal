  ...
  return <div className="quick-add-overlay" role="presentation" onMouseDown={e=>{if(e.currentTarget===e.target)onClose();}}>
    <div className="quick-add-modal card" role="dialog" aria-modal="true" aria-labelledby="quick-add-title">
      ...
      <form onSubmit={submit} className="quick-add-form">
        ...
        <div className="quick-add-two"><label>Category<select value={form.category} onChange={e=>{const category=e.target.value;const key=categoryKey(category);const type:TxType=['rent','other-income','refund'].includes(key)?'income':['contribution','distribution','non-operating'].includes(key)?'transfer':category==='Needs Review'?form.type:'expense';setForm({...form,category,type,needs_review:categoryNeedsReview(category)})}}>{ACCOUNTING_CATEGORIES.map(c=><option key={c}>{c}</option>)}</select></label><label>Date<input required type="date" value={form.transaction_date} onChange={e=>setForm({...form,transaction_date:e.target.value})}/></label></div>
