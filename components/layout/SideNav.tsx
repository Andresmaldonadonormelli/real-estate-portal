...
export default function SideNav() {
  ...
  useEffect(()=>{ let alive=true; (async()=>{
    const [p,t]=await Promise.all([
      ...
      cachedSupabaseRequest('nav:new-import-count',async()=>await supabase.from('transactions').select('id',{count:'exact',head:true}).is('archived_at',null).eq('is_new_import',true).neq('status','declined'))
    ]);
    if(!alive)return;
