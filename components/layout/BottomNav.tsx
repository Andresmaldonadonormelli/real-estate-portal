...
export default function BottomNav({ onMenuClick }:{onMenuClick:()=>void}) {
  ...
  useEffect(()=>{supabase.from('transactions').select('id',{count:'exact',head:true}).is('archived_at',null).eq('is_new_import',true).neq('status','declined').then(({count,error})=>{if(!error)setNewImports(count||0)})},[pathname]);
