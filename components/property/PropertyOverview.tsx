...
const formatKpiCurrency = (value: number) => new Intl.NumberFormat('en-US', {
...
function calculateMetrics(rows: PropertyTransaction[]) {
  let income = 0, operatingExpenses = 0, cashExpenses = 0;
  for (const transaction of rows) { const amount = Math.abs(Number(transaction.amount || 0)); if (transaction.type === 'income') income += amount; if (transaction.type === 'expense') { cashExpenses += amount; if (!OPERATING_EXCLUSIONS.includes(categoryKey(transaction.category || ''))) operatingExpenses += amount; } }
  return { income, operatingExpenses, noi: income - operatingExpenses, cashFlow: income - cashExpenses, cashExpenses };
}

function buildBreakdown(rows: PropertyTransaction[]) {
  const map = new Map<string, { category: string; key: string; amount: number; transactions: PropertyTransaction[] }>();
  for (const transaction of rows) { if (transaction.type !== 'expense') continue; const key = categoryKey(transaction.category || ''); if (OPERATING_EXCLUSIONS.includes(key) || key === 'review') continue; const category = transaction.category || 'Other Expense'; const current = map.get(category) || { category, key, amount: 0, transactions: [] }; current.amount += Math.abs(Number(transaction.amount || 0)); current.transactions.push(transaction); map.set(category, current); }
...
function formatDate(value: string) { if (!value) return ''; const date = new Date(`${value.slice(0, 10)}T12:00:00`); return Number.isNaN(date.getTime()) ? value : date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }); }
