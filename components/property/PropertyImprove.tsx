...
function loanModel(property:Property,_transactions:Tx[],extra:number){
  const p=property,balance=Math.max(0,Number(p.mortgage_balance||0)),payment=Math.max(0,Number(p.mortgage_principal_interest_payment||0)),rate=Math.max(0,Number(p.mortgage_interest_rate||0)),term=Math.max(0,Number(p.mortgage_term_years||0));
  ...
  const baseline=schedule(0),scenario=schedule(extra);
  return{balance,rate,payment,remaining:baseline.months,baseline,scenario};
}
...
function MortgageDetails({model}:{model:ReturnType<typeof loanModel>}){return model?<p>{formatKpiCurrency(model.balance)} balance · {model.rate.toFixed(2)}% rate · {model.remaining} months remaining · {formatKpiCurrency(model.payment)} P&amp;I</p>:<p>Add balance, rate, term, start date and P&amp;I payment.</p>}
function MortgageResults({model,extra}:{model:ReturnType<typeof loanModel>;extra:number}){
  ...
  const max=Math.max(model.baseline.months,model.scenario.months),chartMonths=Math.ceil(max/60)*60,saved=Math.max(0,model.baseline.months-model.scenario.months),interest=Math.max(0,model.baseline.interest-model.scenario.interest),path=(points:Point[])=>points.map(p=>`${p.month/chartMonths*100},${100-p.balance/model.balance*100}`).join(' ');const payoff=new Date();payoff.setMonth(payoff.getMonth()+model.scenario.months);const currentPayoff=new Date();currentPayoff.setMonth(currentPayoff.getMonth()+model.baseline.months);const dateLabel=(date:Date)=>date.toLocaleDateString(undefined,{month:'short',year:'numeric'});const ticks=Array.from({length:chartMonths/60+1},(_,index)=>({p:index*60/chartMonths,l:index?String(index*5)+'Y':'Now'}));
  return <aside className="improve-results improve-mortgage-results"><div className="improve-chart"><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Mortgage balance paths"><polyline className="current" points={path(model.baseline.points)}/><polyline className="scenario" points={path(model.scenario.points)}/></svg><div className="improve-chart-axis">{ticks.map(t=><span key={t.l} style={{left:`${t.p*100}%`}}>{t.l}</span>)}</div><div className="improve-chart-legend"><span><i className="current"/>Current · {dateLabel(currentPayoff)}</span><span><i className="scenario"/>New · {dateLabel(payoff)}</span></div></div><Result label="New payoff date" value={dateLabel(payoff)}/><Result label="Time saved" value={saved>=12?`${Math.floor(saved/12)} yr ${saved%12} mo`:`${saved} mo`}/><Result label="Interest saved" value={formatKpiCurrency(interest)} tone="positive"/><Result label="New monthly payment" value={formatKpiCurrency(model.payment+extra)}/><Result label="Cash-flow reduction today" value={`-${formatKpiCurrency(extra)}`}/></aside>;
}
