...
export default function PropertyExpenseTrendsChart({transactions}:{transactions:Tx[]}){
  ...
  const yearExpense=useMemo(
    ()=>transactions.filter(t=>t.type==='expense'&&t.status!=='declined'&&Number(t.transaction_date.slice(0,4))===currentYear),
    [transactions,currentYear]
  );

  const totals=useMemo(()=>{
    const map=new Map<string,number>();
    yearExpense.forEach(t=>map.set(t.category,(map.get(t.category)||0)+Math.abs(Number(t.amount||0))));
    return map;
  },[yearExpense]);
  ...
  const rows=useMemo(()=>Array.from({length:12},(_,month)=>{
    ...
    const monthRows=yearExpense.filter(t=>t.transaction_date.startsWith(key));
    ...
    if(other>0||monthTotal===0)segments.push({key:'__other',label:'Other',value:other,color:'var(--category-neutral)'});
    return {
      key,
      label:MONTH_LABELS[month],
      fullLabel:`${MONTH_LABELS[month]} ${currentYear}`,
      partial:month===currentMonth,
      total:monthTotal,
      segments,
    };
  }),[yearExpense,topCategories,currentYear,currentMonth]);
  ...
  const axisMax=axisMaximum(Math.max(0,...rows.map(row=>row.total)));
  ...
  return <section className="property-expense-trends">
    ...
    <p className="expense-share-copy">{allTotal?Math.round(topTotal/allTotal*100):0}% of expenses are represented by the top three categories · {currentYear}</p>
    {allTotal?<>
      ...
      <div className="expense-chart" onMouseLeave={()=>setHoveredKey('')}>
        ...
        <div ref={plotRef} className="expense-plot" onPointerDown={event=>{if(event.pointerType==='mouse')return;event.currentTarget.setPointerCapture(event.pointerId);inspectPointer(event)}} onPointerMove={event=>{if(event.currentTarget.hasPointerCapture(event.pointerId))inspectPointer(event)}} onPointerUp={event=>{if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);finishPointer()}} onPointerCancel={finishPointer}>
          {active&&<div className="expense-inspection" data-edge={activeIndex===0?'left':activeIndex===rows.length-1?'right':'center'} style={{left:`${((activeIndex+.5)/Math.max(1,rows.length))*100}%`}}>
            ...
            {active.segments.filter(segment=>segment.value>0||segment.key==='__other').map(segment=><span key={segment.key}><i style={{background:segment.color}} aria-hidden="true"/>{segment.label}<b>{wholeCurrency(segment.value)}</b></span>)}
          ...
          <div className="expense-chart-grid expense-chart-grid-monthly" style={{gridTemplateColumns:`repeat(${rows.length},minmax(0,1fr))`}}>
            {rows.map(row=>{
              ...
              const stackHeight=axisMax?Math.max(row.total?6:0,(row.total/axisMax)*100):0;
              return <button key={row.key} type="button" onMouseEnter={()=>setHoveredKey(row.key)} onFocus={()=>setHoveredKey(row.key)} onBlur={()=>setHoveredKey('')} className={`${activeRow?'active':''} ${inspectionKey&&!activeRow?'is-dimmed':''}`} aria-label={`${row.fullLabel}, ${wholeCurrency(row.total)} total expenses`}>
                <span className="expense-pillar">
                  <span className="expense-pillar-track" aria-hidden="true"/>
                  <span className="expense-pillar-stack" style={{height:`${stackHeight}%`}}>
                    {row.segments.filter(segment=>segment.value>0).map(segment=>{
                      const share=row.total? (segment.value/row.total)*100 : 0;
                      return <i key={segment.key} style={{flexGrow:share,flexBasis:0,background:segment.color}} title={`${segment.label}: ${wholeCurrency(segment.value)}`}/>;
                    })}
                  </span>
                </span>
                <em>{row.label}</em>
