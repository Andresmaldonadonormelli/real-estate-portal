...
export default function FinancialHistoryChart({rows,mode='cashFlow',label,onInspect}:{rows:MonthlyFinancialPoint[];mode?:HistoryMode;label:string;onInspect?:(row:MonthlyFinancialPoint|null)=>void}){
  ...
  return <div className="financial-history-chart-wrap">
    ...
    <div className="financial-history-axis" style={{gridTemplateColumns:`repeat(${Math.max(1,rows.length)},minmax(0,1fr))`}} aria-hidden="true">{rows.map((row,index)=><span key={row.key}>{rows.length<=12||index%Math.ceil(rows.length/6)===0||index===rows.length-1?row.label:''}</span>)}</div>
  </div>;
}
