...
import { Banknote, Landmark, Wrench, Zap, ShieldCheck, Receipt, FileText, Building2, Hammer, Scale, WalletCards, CircleDollarSign, ClipboardCheck, RotateCcw, BadgeDollarSign, Paperclip, ChevronRight } from 'lucide-react';
...
export default function LedgerTab({ selectedPropertyId }:{ selectedPropertyId:string }) {
  ...
  return <div>
    ...
    <div className="ledger-view-actions"><button onClick={()=>setViewMode('table')} className={viewMode==='table'?'ledger-control active':'ledger-control'}>Table</button><button onClick={()=>setViewMode('months')} className={viewMode==='months'?'ledger-control active':'ledger-control'}>Months</button><button onClick={exportCsv} className="ledger-control">Export</button>{reviewOnly&&<span className="review-filter-chip">Needs Review · {filtered.length}</span>}</div>
    ...
    {loading?<PageSkeleton variant="ledger"/>:filtered.length===0?<div className="card ledger-empty-state"><strong style={{display:'block',color:'var(--text-primary)',marginBottom:5}}>No transactions found</strong><span>Try clearing a filter or add a transaction.</span></div>:viewMode==='months'?<div style={{display:'grid',gap:12}}>{groups.map(group=>{const totals=calculateMonthlyTotals(group.transactions);const open=expandedMonths.has(group.key);return <div className="card" key={group.key} style={{padding:0,overflow:'hidden'}}><button onClick={()=>setExpandedMonths(prev=>{const n=new Set(prev);n.has(group.key)?n.delete(group.key):n.add(group.key);return n;})} className="ledger-month-head"><div><strong>{formatMonthYear(group.year,group.month)}</strong><span>{group.transactions.length} txns</span><small>Income <b className="amount-positive">{formatCurrency(totals.income)}</b> · Expenses <b className="amount-negative">{formatCurrency(totals.expense)}</b></small></div><div><strong className={totals.net>=0?'amount-positive':'amount-negative'}>{formatCurrency(totals.net)}</strong><span>{open?'⌃':'⌄'}</span></div></button>{open&&<div style={{borderTop:'1px solid var(--border-color)'}}>{group.transactions.map(tx=><TxRow key={tx.id} tx={tx} property={propertyName(tx.property_id)} unit={unitName(tx.unit_id)} attachmentCount={(attachmentCounts[tx.id]||0)+((tx as any).receipt_path?1:0)} onEdit={()=>openEdit(tx)}/>)}</div>}</div>})}</div>:<><div className="card ledger-table-wrap"><table className="ledger-table"><thead><tr><th>Date</th><th>Property / Unit</th><th>Description</th><th>Category</th><th>Payee</th><th>Amount</th><th></th></tr></thead><tbody>{filtered.map(tx=>{const needsReview=Boolean((tx as any).needs_review)||categoryNeedsReview(tx.category);const attachments=(attachmentCounts[tx.id]||0)+((tx as any).receipt_path?1:0);return <tr key={tx.id} className="ledger-clickable-row" tabIndex={0} onClick={()=>openEdit(tx)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openEdit(tx)}}}><td>{formatDateShort(tx.transaction_date)}</td><td>{propertyName(tx.property_id)}{unitName(tx.unit_id)?` · ${unitName(tx.unit_id)}`:''}</td><td><div className="ledger-description-cell"><span>{tx.description}</span>{attachments>0&&<span className="ledger-paperclip" title={`${attachments} supporting ${attachments===1?'document':'documents'}`}><Paperclip size={13}/>{attachments}</span>}</div></td><td><div className="ledger-table-category">{categoryKey(tx.category)!=='review'&&<CategoryIcon category={tx.category}/>}<span>{categoryKey(tx.category)==='review'?'Uncategorized':tx.category}</span>{needsReview&&<span className="needs-review-badge">Needs review</span>}</div></td><td>{tx.payee_source||'—'}</td><td className={tx.type==='income'?'amount-positive':tx.type==='expense'?'amount-negative':''}>{formatCurrency(tx.amount)}</td><td className="ledger-row-chevron"><ChevronRight size={17}/></td></tr>})}</tbody></table></div><div className="ledger-mobile-list">{filtered.map(tx=>{const needsReview=Boolean((tx as any).needs_review)||categoryNeedsReview(tx.category);const attachments=(attachmentCounts[tx.id]||0)+((tx as any).receipt_path?1:0);return <div key={tx.id} className="card ledger-mobile-card"><div className="ledger-mobile-content"><div className="ledger-mobile-meta">{formatDateShort(tx.transaction_date)} · {propertyName(tx.property_id)}{unitName(tx.unit_id)?` · ${unitName(tx.unit_id)}`:''}</div><strong>{tx.description}</strong><div className="ledger-mobile-category">{categoryKey(tx.category)!=='review'&&<CategoryIcon category={tx.category}/>}<span>{categoryKey(tx.category)==='review'?'Uncategorized':tx.category}{tx.payee_source?` · ${tx.payee_source}`:''}</span>{needsReview&&<span className="needs-review-badge">Needs review</span>}{attachments>0&&<span className="ledger-paperclip"><Paperclip size={12}/>{attachments}</span>}</div></div><div className="ledger-mobile-right"><strong className={tx.type==='income'?'amount-positive':tx.type==='expense'?'amount-negative':''}>{formatCurrency(tx.amount)}</strong><button type="button" onClick={()=>openEdit(tx)} aria-label="Open transaction"><ChevronRight size={18}/></button></div></div>})}</div></>}
