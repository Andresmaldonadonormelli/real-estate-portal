...
export default function LedgerTab({ selectedPropertyId }:{ selectedPropertyId:string }) {
  ...
  return <div>
    ...
    <div className="ledger-view-actions"><div className="ledger-view-mode-group"><button onClick={()=>setViewMode('table')} className={viewMode==='table'?'ledger-control active':'ledger-control'}>Table</button><button onClick={()=>setViewMode('months')} className={viewMode==='months'?'ledger-control active':'ledger-control'}>Months</button><button onClick={exportCsv} className="ledger-control">Export</button></div>{reviewCount>0&&<label className="ledger-review-toggle"><input type="checkbox" checked={reviewFilter} onChange={e=>setReviewFilter(e.target.checked)}/><span className="ledger-review-switch" aria-hidden="true"><i/></span><span className="ledger-review-label">Needs review</span><em>{reviewCount}</em></label>}</div>
      ...
      {filtered.map(tx=>{
        ...
        return <button type="button" key={tx.id} className="ledger-modern-row" onClick={()=>openEdit(tx)}>
          ...
          <span className="ledger-modern-main">
            ...
            <span className="ledger-mobile-posted-date">{formatDateShort(tx.transaction_date)}</span>
          </span>
