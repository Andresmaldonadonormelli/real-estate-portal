...
export default function LedgerTab({ selectedPropertyId }:{ selectedPropertyId:string }) {
  ...
  async function saveTx(e:FormEvent){
    ...
    const payload={user_id:auth.user.id,property_id:form.property_id,unit_id:form.unit_id||null,transaction_date:form.transaction_date,type:form.type,category:form.category,description:form.description.trim(),payee_source:form.payee_source.trim()||null,amount:form.type==='expense'?-entered:entered,notes:form.notes.trim()||null,source:'manual',import_key:null,status:'posted',confirmed_at:new Date().toISOString()};
  ...
  async function importCsv(){
    ...
    const {data:auth}=await supabase.auth.getUser();
    if(!auth.user){setError('You are not signed in.');setImporting(false);return;}
    const payload=previewRows.map(p=>({user_id:auth.user!.id,property_id:p.propertyId,unit_id:p.unitId,transaction_date:p.date,type:p.type,category:p.category,description:p.description,payee_source:p.payee,amount:p.amount,notes:p.notes,source:'doorvest_csv',import_key:p.importKey,status:'posted',confirmed_at:new Date().toISOString()}));
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'flex-end',alignItems:'center',gap:8,marginBottom:18,flexWrap:'wrap'}}><button onClick={openImport} disabled={!properties.length} style={secondaryButton}>Import CSV</button><button onClick={openAdd} disabled={!properties.length} style={primaryButton}>+ Add transaction</button></div>
