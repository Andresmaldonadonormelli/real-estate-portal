...
import { Banknote, Landmark, Wrench, Zap, ShieldCheck, Receipt, FileText, Building2, Hammer, Scale, WalletCards, CircleDollarSign, ClipboardCheck, RotateCcw, BadgeDollarSign, Paperclip } from 'lucide-react';
...
export default function LedgerTab({ selectedPropertyId }:{ selectedPropertyId:string }) {
  ...
  const filtered = useMemo(()=>transactions.filter(tx=>{
    ...
    if(reviewOnly && !(Boolean((tx as Transaction & {needs_review?:boolean}).needs_review)||categoryNeedsReview(tx.category))) return false;
    ...
    return true;
  }),[transactions,filters,selectedPropertyId,reviewOnly]);
  ...
  function openAdd(){setEditing(null);setReceiptFile(null);setForm({...emptyTx,property_id:selectedPropertyId||properties[0]?.id||''});setShowForm(true);}
  function openEdit(tx:Transaction){setEditing(tx);setReceiptFile(null);setForm({property_id:tx.property_id,unit_id:tx.unit_id||'',transaction_date:tx.transaction_date,type:tx.type,category:tx.category,description:tx.description,payee_source:tx.payee_source||'',amount:String(Math.abs(tx.amount)),notes:tx.notes||'',needs_review:Boolean((tx as Transaction & {needs_review?:boolean}).needs_review)||categoryNeedsReview(tx.category)});setShowForm(true);}
  ...
  return <div>
    ...
    {reviewOnly&&<div className="review-filter-banner"><strong>Needs review</strong><span>Showing transactions that still need accounting categorization.</span></div>}
...
function TxRow({tx,property,unit,onEdit,onDelete}:{tx:Transaction;property:string;unit:string;onEdit:()=>void;onDelete:()=>void}){const pending=tx.status==='pending';const needsReview=Boolean((tx as Transaction & {needs_review?:boolean}).needs_review)||categoryNeedsReview(tx.category);return <div className="ledger-tx-row" style={{display:'grid',gridTemplateColumns:'auto minmax(0,1fr) auto',gap:12,padding:'14px 18px',borderBottom:'1px solid var(--border-color)',opacity:pending?0.72:1}}><CategoryIcon category={tx.category}/><div><div style={{fontSize:13,color:'var(--text-secondary)'}}>{formatDateShort(tx.transaction_date)} · {property}{unit?` · ${unit}`:''}</div><div style={{fontWeight:550,marginTop:3}}>{tx.description}{needsReview&&<span className="needs-review-badge">Needs review</span>}{pending&&<span style={{marginLeft:8,fontSize:11,padding:'2px 6px',border:'1px solid var(--border-color)',borderRadius:999,color:'var(--text-secondary)'}}>Pending</span>}</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:3}}>{tx.category}{tx.payee_source?` · ${tx.payee_source}`:''}{((tx as any).receipt_path||(tx as any).supporting_document_id)&&<span className="ledger-attachment" title="Supporting document attached"><Paperclip size={12}/> Attached</span>}</div></div><div style={{textAlign:'right'}}><div style={{fontWeight:600,color:pending?'var(--text-secondary)':tx.type==='income'?'var(--positive)':tx.type==='expense'?'var(--danger)':'var(--text-secondary)'}}>{formatCurrency(tx.amount)}</div><div style={{display:'flex',gap:5,marginTop:6}}><button onClick={onEdit} style={smallButton}>Edit</button><button className="danger-action" onClick={onDelete} style={{...smallButton,color:'var(--danger)'}}>{tx.source==='recurring'?'Skip month':'Archive'}</button></div></div></div>}
