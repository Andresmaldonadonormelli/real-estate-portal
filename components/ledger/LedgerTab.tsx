...
function CategoryIcon({category}:{category:string}){const props={size:19,strokeWidth:1.8};const Icon=category==='Rent'?Banknote:category==='Mortgage'?Landmark:category==='Repairs & Maintenance'?Wrench:category==='Utilities'?Zap:category==='Insurance'?ShieldCheck:category==='Management Fee'?ClipboardCheck:category==='Leasing Fee'?Receipt:category==='Property Taxes'?Building2:category==='CapEx'?Hammer:category==='Legal'?Scale:category==='Owner Distribution'?WalletCards:category==='Other Income'?CircleDollarSign:category.toLowerCase().includes('refund')?RotateCcw:FileText;return <span className="ledger-category-icon" aria-hidden="true"><Icon {...props}/></span>}
