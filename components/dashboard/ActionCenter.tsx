...
export default function ActionCenter({items,title='Action Center',showEyebrow=true,hideWhenEmpty=false,onViewAll}:{items:ActionCenterItem[];title?:string;showEyebrow?:boolean;hideWhenEmpty?:boolean;onViewAll?:()=>void}){
  ...
  return <section className="pulse-action-section shared-action-center">
    <div className="pulse-action-center">
      ...
      {items.length?<div className="action-list">{items.slice(0,3).map(item=><button key={item.id} className="action-row" onClick={item.onSelect}><ActionIcon item={item}/><span><strong>{item.title}</strong><small>{item.detail}</small></span><em className="pulse-row-action">{item.kind==='rent'?'Confirm':'Review'}</em></button>)}</div>:<div className="pulse-all-clear"><strong>All clear</strong><span>No tasks need attention.</span></div>}
    </div>
  </section>;
}
